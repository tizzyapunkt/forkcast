...
Surface Open Food Facts serving-size metadata through ingredient search and use it to reduce friction in the log-ingredient flow. Macro values are always derived from the reliable per-100g basis, while the optional per-serving size and quantity are carried as additive metadata that can pre-fill the gram input — never altering any macro calculation.
