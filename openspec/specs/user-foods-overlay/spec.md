...
Provide a runtime-writable overlay of user-confirmed foods and learned synonyms that complements the curated FOODS catalog. Foods confirmed during import review or picker search are persisted to a JSON overlay, made immediately searchable as the `USER` source, and learned synonyms extend the curated index without a backend restart. The overlay is drained via an atomic export-and-clear endpoint that feeds the laptop curation pipeline.
