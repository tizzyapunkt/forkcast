...
Gate the entire forkcast application behind a single shared password using a JWT stored in an `HttpOnly; SameSite=Strict` cookie. The backend authenticates a submitted password against an environment-provided secret, issues a signed session cookie on success, and rejects unauthenticated requests on every API route except the auth endpoints. The frontend renders a login-only shell when no valid session is present and the full app shell when one is.
