...
Give the backend a self-observing record that survives the trip from a phone to a Claude Code session. Developing forkcast on the go means no browser console and no reachable server logs, so a failure in production — an Open Food Facts call that quietly returns nothing, a handler that throws — leaves no evidence behind. A bounded in-memory ring buffer captures HTTP requests, handler errors, and outbound Open Food Facts calls, and an auth-protected endpoint hands them back as JSON. The buffer is deliberately in-memory and resets on restart: this is a diagnostics window, not an audit trail.
