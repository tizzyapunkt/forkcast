...
Derive daily calorie and macro goals from body metrics, activity factor, fitness phase, and calorie adjustment percentage using the Ten Haaf & Weijs (2014) body-weight REE formula. Persist the calculator inputs (as a `BodyProfile`) independently of the active `DailyGoal`, and provide an explicit action to write the computed result into the existing daily nutrition goal storage.
