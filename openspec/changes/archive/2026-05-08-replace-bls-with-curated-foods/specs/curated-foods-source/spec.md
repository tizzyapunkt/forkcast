...
The repository SHALL ship a TypeScript build script (`backend/scripts/build-foods-data.ts`) that reads a curated, hand-edited list of food keys (`backend/scripts/foods-seed-keys.ts`) and produces `backend/data/foods.json`. The script SHALL:
...
- Validate every output entry against the requirements above (canonical name, synonyms shape, unit, macros, optional pieces) and fail the run if any entry fails validation.
