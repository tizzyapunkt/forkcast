...
The two catalog-side sources (`foods`, `user`) collapse into one (`catalog`), and the toggle inverts: the user's own catalog is always searched, while Open Food Facts becomes the opt-in source. This fixes a standing defect — the panel defaulted to OFF-only and never requested `user` at all, so a food the user had confirmed was unreachable from the search box.
