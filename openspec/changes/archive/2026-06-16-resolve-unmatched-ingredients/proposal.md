...
- **Create with AI from the search panel**: when a picker/search query yields no suitable result, an affordance triggers the same propose → show-and-confirm → overlay flow for that single query, and the confirmed entry is delivered to the host flow as a normal search selection. This closes the wrong-match case (swap a mismatched row to a food that doesn't exist yet) and works wherever the search panel is used.
...
- **Frontend**: review screen unmatched panel gains the resolve flow (batch prefetch, per-item confirm sheet — layout to be supplied by a separate design handoff); the shared search panel gains a create-with-AI affordance reusing the same confirm sheet; settings panel swaps unmatched export for overlay export; deletion of `features/unmatched-ingredients` + related api/queries.
