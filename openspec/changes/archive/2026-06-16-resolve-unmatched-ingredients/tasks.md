...
- [x] 9.1 Smoke test — `make smoke` verifies the backend round-trip (confirm → search → export-and-clear). Live browser e2e (via `make dev-http` + Chrome tools) exercised the create-with-AI path against the real model: search off-catalog → propose (live, untracked steering confirmed for a seasoning) → confirm → persisted to overlay (untracked, zero macros) → handed to the amount step. Caught + fixed a blank-sheet bug (draft seeded only at mount; now seeded when the proposal arrives) with a regression test. Photo-upload entry point not driven from automation (browser file-upload tooling limitation) but exercised in real-device testing and shares the verified resolve machinery.
