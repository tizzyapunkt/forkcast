...
- **Spoon conversion has a defined order on tracked matches.** An `ml` food gets the fixed spoon volume. A `g` food with a `density` gets volume × density. A `g` food without density gets count × `gramsPerSpoon`. An estimate heavier than 1.5 g per ml of its spoon (denser than honey, so the model mixed up TL and EL) is ignored. Only when none of these apply does the row stay `missingAmount`.
