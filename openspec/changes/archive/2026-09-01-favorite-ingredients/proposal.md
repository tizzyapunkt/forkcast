...
- A star toggle appears on every **search result row**, every **recent row**, and every **favorites row**, in both the log drawer and the recipe ingredient picker. Tapping the star toggles favorite state without selecting the row; on the Favorites tab it removes the favorite in place, so unfavoriting never means hunting the ingredient down in Search or Recent. The star stays enabled on rows whose selection is gated (untracked seasonings), since favoriting one is still useful for recipes.
