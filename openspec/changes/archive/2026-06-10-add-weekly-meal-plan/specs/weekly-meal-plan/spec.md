...
deletes that one entry. Because the planner and diary share the same data, an add or remove in the
planner MUST be reflected in the diary for that date (and vice versa) without a manual refresh.
