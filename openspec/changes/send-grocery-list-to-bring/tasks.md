...
- [ ] 3.1 Write failing tests for the page renderer: Recipe microdata with `itemscope`/`itemtype`, name with the week range, `yield` 1, one `ingredients` line per item in order, German unit labels, HTML escaping of names, empty list renders no lines; implement until green
- [ ] 3.2 Write failing handler tests for `GET /bring-import/:token`: `200` HTML without a session, excluded identities left out (case-insensitive), `Cache-Control: no-store` and `X-Robots-Tag: noindex`, `401` for expired/tampered/malformed tokens with no list content; implement until green
- [ ] 3.3 Write failing handler tests for `POST /bring-import-token`: `200` with a token, `400` for malformed `startDate`/`excluded`; implement until green
- [ ] 3.4 Register `GET /bring-import/:token` **before** and `POST /bring-import-token` **after** the auth middleware in `backend/src/index.ts`; add an auth-middleware test that an import token used as a session cookie yields `401`
- [ ] 3.5 Smoke-test via the dev proxy: mint a token with a session, fetch `/api/bring-import/{token}` without one, and check the HTML

## 4. Frontend

- [ ] 4.1 Add `api/bring-import-token.ts`, an MSW handler, and German copy ("An Bring! senden", error text); verify typecheck
...
- [ ] 5.1 Run backend and frontend test, lint, typecheck and format per the forkcast-dev skill; all green
