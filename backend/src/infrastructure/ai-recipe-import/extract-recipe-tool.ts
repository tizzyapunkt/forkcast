...
import { spoonVolumeMl } from '../../domain/ai-recipe-import/convert-spoon-amount.ts';

export const EXTRACT_RECIPE_TOOL_NAME = 'extract_recipe';

// The model reads, the code converts: `amount`/`unit` only ever carry a printed mass or volume.
// Counts go through the piece fields (a stray unit: 'piece' is reconciled to grams in
// buildPieceQuantity) and spoons/cups through the raw-display fields, where the domain converts
// them. Spoon units the model still puts on `unit` are moved there by normalizeSpoonMeasure.
const CANONICAL_UNITS: MeasurementUnit[] = ['g', 'ml'];

/** German label for a spoon the model reported as an enum unit — the catalog locale, as on untracked rows. */
const SPOON_UNIT_LABEL: Readonly<Record<string, string>> = { tbsp: 'EL', tsp: 'TL', cup: 'Tasse' };
        ...
        items: {
          ...
          properties: {
            ...
            amount: {
              type: 'number',
              description:
                'Numeric amount in mass (g) or volume (ml) units — only a weight or volume the recipe prints, or a piece total. When piece fields are populated, this MUST equal pieceAmount * gramsPerPiece. A spoon or cup count ("2 EL") is not an amount here; it goes on rawDisplayAmount. OMIT entirely if the recipe does not quantify the ingredient (e.g. "salt to taste").',
            },
            unit: {
              type: 'string',
              enum: CANONICAL_UNITS,
              description:
                'Measurement unit. When piece fields are populated, MUST be "g" (or "ml" only if the recipe explicitly frames the piece as a liquid quantity, e.g. "juice of 1 lemon"). Spoons and cups (EL, TL, Tasse) are never a unit here; they go on rawDisplayUnitLabel. OMIT entirely if not shown or not in the supported set.',
            },
            pieceAmount: {
              type: 'number',
              description:
            ...
            rawDisplayAmount: {
              type: 'number',
              description:
                'Literal numeric amount as written in the recipe, when the recipe uses a unit outside the canonical enum (typical for seasonings/spices/herbs, e.g. "1 TL", "2 EL", "1/2 Prise"). Fractional values are permitted. OMIT when the recipe states the ingredient in g or ml.',
            },
            rawDisplayUnitLabel: {
              type: 'string',
              description:
            ...
            gramsPerSpoon: {
              type: 'number',
              description:
                'Your best estimate of the mass in grams of ONE level spoon or cup — as named in rawDisplayUnitLabel — of this specific food (e.g. 1 EL Haferflocken ≈ 8, 1 EL Erdnussmus ≈ 16, 1 EL Olivenöl ≈ 13, 1 TL Zimt ≈ 2.5). Populate whenever rawDisplayUnitLabel is a spoon or cup (EL, TL, Esslöffel, Teelöffel, Tasse). OMIT otherwise.',
            },
            note: {
              type: 'string',
              description:
...
export const EXTRACT_RECIPE_INSTRUCTIONS = [
  ...
  '- rawDisplayAmount/rawDisplayUnitLabel are ONLY for MEASURING units that are not counts and fall outside the canonical enum — spoons, pinches, splashes and the like (e.g. "1 TL Salz", "2 EL Olivenöl", "Prise Pfeffer", "Schuss Zitronensaft", "Salz n. Geschmack"). Populate them with the literal numeric amount and textual unit as written in the original language. Leave amount and unit empty for a spoon or cup measure — the app converts it — unless the recipe itself prints a weight or volume next to it (e.g. "2 EL (12 g) Speisestärke" → also amount 12, unit "g"). For every spoon or cup measure, also populate gramsPerSpoon with your estimate of one spoon of that food in grams. Do NOT route a counted food or a size-described piece through these fields — those take the piece fields above.',
  ...
  'Worked examples — piece path (counted foods, incl. size-described): "½ mittelgroße Zucchini" → {name:"Zucchini", pieceAmount:0.5, pieceUnitLabel:"Zucchini", gramsPerPiece:200, amount:100, unit:"g"}; "¼ mittelgroße rote Zwiebel" → {name:"rote Zwiebel", pieceAmount:0.25, pieceUnitLabel:"Zwiebel", gramsPerPiece:120, amount:30, unit:"g"}; "1 Zehe Knoblauch" → {name:"Knoblauch", pieceAmount:1, pieceUnitLabel:"Zehe", gramsPerPiece:5, amount:5, unit:"g"}. Raw-display path (measuring units only): "2 EL Olivenöl" → {name:"Olivenöl", rawDisplayAmount:2, rawDisplayUnitLabel:"EL", gramsPerSpoon:13}; "½ TL Kreuzkümmel" → {name:"Kreuzkümmel", rawDisplayAmount:0.5, rawDisplayUnitLabel:"TL", gramsPerSpoon:2}; "2 EL (12 g) Speisestärke" → {name:"Speisestärke", amount:12, unit:"g", rawDisplayAmount:2, rawDisplayUnitLabel:"EL", gramsPerSpoon:6}.',
...
function isSpoonLabel(label: unknown): label is string {
  return typeof label === 'string' && spoonVolumeMl(label) !== undefined;
}

function hasRawDisplay(ing: RawToolInputIngredient): boolean {
  return (
    ing.rawDisplayAmount !== undefined ||
    (typeof ing.rawDisplayUnitLabel === 'string' && ing.rawDisplayUnitLabel.trim().length > 0)
  );
}

/**
 * Move every spoon/cup measure onto the raw-display fields, however the model shaped it, so the
 * domain converts all of them by one rule. Order matters — a label the model wrote beats one
 * derived from an enum value:
 *
 * 1. Spoon-labelled piece fields ("2 EL" as a piece) become a spoon, gramsPerPiece its estimate.
 * 2. A spoon enum unit (tbsp/tsp/cup) becomes a spoon count with the German label.
 * 3. An ml amount next to a spoon is the model's own conversion and is dropped (a printed g stays).
 * 4. Ounces are a mass and become grams.
 *
 * Structured fields only: `sourceText` is the verbatim read and is never read or rewritten here.
 */
function normalizeSpoonMeasure(input: RawToolInputIngredient): RawToolInputIngredient {
  const ing: RawToolInputIngredient = { ...input };

  if (isSpoonLabel(ing.pieceUnitLabel)) {
    if (!hasRawDisplay(ing)) {
      ing.rawDisplayAmount = ing.pieceAmount;
      ing.rawDisplayUnitLabel = ing.pieceUnitLabel;
    }
    if (ing.gramsPerSpoon === undefined) ing.gramsPerSpoon = ing.gramsPerPiece;
    ing.pieceAmount = ing.pieceUnitLabel = ing.gramsPerPiece = undefined;
    ing.amount = ing.unit = undefined;
  }

  if (typeof ing.unit === 'string' && SPOON_UNIT_LABEL[ing.unit] !== undefined) {
    if (!hasRawDisplay(ing)) {
      ing.rawDisplayAmount = ing.amount;
      ing.rawDisplayUnitLabel = SPOON_UNIT_LABEL[ing.unit];
    }
    ing.amount = ing.unit = undefined;
  }

  if (ing.unit === 'ml' && isSpoonLabel(ing.rawDisplayUnitLabel)) {
    ing.amount = ing.unit = undefined;
  }
  ...
  if (ing.unit === 'oz') {
    ...
    if (typeof ing.amount === 'number' && Number.isFinite(ing.amount))
      ing.amount = round1(ing.amount * GRAMS_PER_OUNCE);
  }
...
export function parseToolInput(input: unknown): ExtractedDraft {
  ...
  const ingredients: RawIngredient[] = raw.ingredients.map((item, idx) => {
    ...
    const ing = normalizeSpoonMeasure(item as RawToolInputIngredient);
    ...
    if (typeof ing.unit === 'string' && (CANONICAL_UNITS as string[]).includes(ing.unit)) {
    ...
    // An estimate only means something for a spoon/cup the domain knows how to measure.
    if (isFinitePositive(ing.gramsPerSpoon) && isSpoonLabel(result.rawDisplayUnitLabel)) {
      result.gramsPerSpoon = ing.gramsPerSpoon;
    }
