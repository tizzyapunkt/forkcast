...
- [ ] **Macro color tokens**: add `--macro-p / --macro-c / --macro-f` + brightened `--macro-p-on /
...
- [ ] **Diary/planner header macro bars**: give the three macro cells the design's bar + label-dot
...
- [ ] Slot-card kcal total: render in `--primary` / font-weight 700 (currently plain dark text).
- [ ] Slot-card "+" button: 44px soft-accent square (rounded, `--accent-soft` bg, primary plus icon)
      — currently a bare glyph. Planner already uses the soft square; diary should match it.
- [ ] Weight card: add the scale icon next to "Heutiges Gewicht"; link label "Gewicht-Tracker"
      (currently "Zum Gewicht-Tracker"); h2 at 16.5px to match meal-slot headings (checklist item 5).
- [ ] Weight input placeholder: "z. B. 78,4" (German comma; currently "78.4").
- [ ] Date label format: "Do. 11. Juni" without comma (currently "Do., 11. Juni") to match the design.

## Add-food sheet

- [ ] AmountStep: render the live total as a separate summary card (muted bg, kcal 24px/800 in
      `--primary` left, macro triplet right) instead of a text line inside the food card.
- [ ] AmountStep confirm button: add the check icon (label already includes the amount ✓).
- [ ] Rezepte tab rows: add right-aligned "{kcal} kcal/P" figure and a chevron (currently name +
      "N Zut. · N Port." only).
- [ ] RecipePortionStep: portions as a stepper (−/+) instead of a raw number input; add the live
...
- [ ] Toolbar: two equal-width (flex:1) buttons — ghost "Aus Fotos" with camera icon, primary "Neu"
      (currently right-aligned next to the heading, labeled "+ Neues Rezept", no camera icon).
- [ ] Recipe card: 44×44 rounded icon tile (`--accent-soft` bg, cook glyph in `--primary`) + kcal in
...
- [ ] Actions row: right-aligned as the FIRST element of the scroll body — ghost "Bearbeiten" with
      pencil icon + danger trash ICON button (currently "Löschen" text button, placed above the title).
- [ ] Pro-Portion card: header strip with `--accent-soft` bg, uppercase 11px eyebrow "PRO PORTION" in
      `--primary` (currently a plain two-row light card).
- [ ] Drop the extra "Bei N Portionen" second row (not in the design; the Portionen stepper scales
...
- [ ] Add the faint "Portionen" caption left of the stepper in the Zutaten heading row.

## Rezept-Editor

- [ ] Hero card macro row: three colored-dot items "● Eiweiß {n} g · ● KH {n} g · ● Fett {n} g"
      using the `--macro-*` tokens (currently an inline text triplet next to the kcal number).
- [ ] Hero card fill: subtle `linear-gradient(180deg, --accent-soft, --card)`.
- [ ] "Zutaten · N" heading with live ingredient count (currently "Zutaten").
- [ ] Ingredient note: collapsed by default behind a subtle "+ Notiz" ghost text button (plain text,
...
- [ ] Keep the existing ↻ "Zutat ersetzen" control (app feature, not in the design) — place it in the
      title row next to "+ Notiz" / remove without breaking the design's row layout.

## Wochenplan (planner)

- [ ] Day cards: show the date under the weekday badge ("Mo" / "1. Juni" — currently bare day
      number); macro triplet right-aligned on the kcal row (currently below it).
- [ ] Expanded day: add the "TAG GESAMT" block (muted card with the three colored macro bars) above
      the meal slots.
- [ ] Week rollups: style as the design's stat layout ("585 Ø kcal/Tag" prominent, "4/7 Tage geplant",
...
- [ ] "Ernährungsziel" as h2 over a card containing the goal inputs; kcal full-width, the three macro
