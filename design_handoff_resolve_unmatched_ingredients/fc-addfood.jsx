...
(function () {
  ...
  function SearchTab({ onPick, onCreate }) {
    const [q, setQ] = useState('Brot');
    const query = q.trim();
    const results = query ? D.SEARCH_BROT.filter((it) => it.name.toLowerCase().includes(query.toLowerCase())) : [];
    return (
      <div>
        ...
        <div className="fc-divide" style={{ marginTop: 6 }}>
          ...
          {!query && <p className="fc-faint" style={{ fontSize: 14, padding: '14px 2px' }}>Tippe, um Zutaten zu suchen.</p>}
          {query && results.length === 0 && (
            <p className="fc-muted" style={{ fontSize: 14, padding: '16px 2px 4px' }}>Kein Treffer für „{query}“ im Katalog.</p>
          )}
        </div>
        {/* persistent create trigger — the second host of the show-and-confirm sheet */}
        {query && (
          <button onClick={() => onCreate(query)}
            style={{ width: '100%', marginTop: results.length === 0 ? 6 : 12, display: 'flex', alignItems: 'center', gap: 10,
            ...
            <span style={{ width: 30, height: 30, borderRadius: 9, background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <Icon name="plus" size={18} color="var(--primary)" />
            </span>
            <span style={{ minWidth: 0, flex: 1 }}>
              <span style={{ display: 'block', fontSize: 14.5, fontWeight: 650, color: 'var(--primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>„{query}“ neu anlegen</span>
              <span className="fc-faint" style={{ fontSize: 12 }}>Eigenen Eintrag erstellen · sofort nutzbar</span>
            </span>
            <Icon name="chevR" size={18} color="var(--primary)" />
          </button>
        )}
      </div>
    );
  ...
  function AddFoodSheet({ open, slot, onClose, onAdd, expandRecipes }) {
    ...
    const [createItem, setCreateItem] = useState(null);   // "nichts gefunden" → reusable create sheet
    function reset() { setAmountItem(null); setRecipeItem(null); setCreateItem(null); }
    function finish(entry) { onAdd(entry); onClose(); setTimeout(reset, 300); }

    // a confirmed/created food drops into the amount step so the user logs a quantity
    function onCreated(food) {
      setCreateItem(null);
      setAmountItem({ name: food.name, unit: food.unit, kcal: Number(food.per100.kcal) || 0,
        p: Number(food.per100.p) || 0, c: Number(food.per100.c) || 0, f: Number(food.per100.f) || 0 });
    }
    ...
    return (
      <Sheet open={open} onClose={() => { onClose(); setTimeout(reset, 300); }}
        ...
        {amountItem ? (
          ...
          <window.ResolvePane
            context="create"
            item={{ name: createItem.name, unit: 'g' }}
            state="ready"
            showConfidence={false}
            proposal={{ verdict: 'new-food', confidence: null, entry: { name: createItem.name, unit: 'g', untracked: false, per100: { kcal: '', p: '', c: '', f: '' } } }}
            onConfirm={onCreated}
          ...
          <>
            ...
            {tab === 'search' && <SearchTab onPick={setAmountItem} onCreate={(name) => setCreateItem({ name })} />}
