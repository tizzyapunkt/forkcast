...
(function () {
  ...
  // ───────── one ingredient row ─────────
  function IngredientRow({ r, measureStyle, tag, highlight, onPatch, onRemove }) {
    ...
    return (
      <li style={{ padding: '14px 0', display: 'flex', flexDirection: 'column', gap: 10,
        animation: highlight ? 'fc-row-in .5s cubic-bezier(.2,.7,.3,1) both' : undefined }}>
        {/* title row */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ flex: 1, minWidth: 0, display: 'inline-flex', alignItems: 'center', gap: 7 }}>
            <span style={{ minWidth: 0, fontSize: 15.5, fontWeight: 600, color: r.untracked ? 'var(--muted-fg)' : 'var(--fg)',
              overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.name}</span>
            {tag && <span className="fc-chip" style={{ flexShrink: 0, fontSize: 9.5, padding: '2px 6px', letterSpacing: '.04em',
              background: tag === 'USER' ? 'var(--accent-soft)' : 'var(--muted)', color: tag === 'USER' ? 'var(--primary)' : 'var(--muted-fg)' }}>{tag}</span>}
          </span>
          {!noteOpen && (
  ...
  Object.assign(window, { RecipesScreen, RecipeDetail, RecipeEditor, IngredientRow, FCSimpleHeader: SimpleHeader });
})();
